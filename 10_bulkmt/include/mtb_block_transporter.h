...
private:
    ...
    std::unique_ptr<MtbTransporterWorkerStd>  worker_stdout_;
    std::unique_ptr<MtbTransporterWorkerFile> worker_file_1_;
    std::unique_ptr<MtbTransporterWorkerFile> worker_file_2_;

    std::shared_ptr<std::condition_variable> cv_stdout_;
    std::shared_ptr<std::condition_variable> cv_file_x_;

    std::shared_ptr<std::mutex> mtx_stdout_;
    std::shared_ptr<std::mutex> mtx_file_x_;

    std::shared_ptr<std::queue<DataBlock>> block_queue_stdout_;
    std::shared_ptr<std::queue<DataBlock>> block_queue_file_x_;
};
